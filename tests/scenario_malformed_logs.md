...
and routed immediately to the Dead Letter Queue, while transient downstream
failures are retried — without
...
3. Watch consumer output for validation errors and DLQ routing:
   ```
  event={"event": "dlq_publish", "retry_count": 0, ...}
   ```
...
| DLQ `retry_history`           | Exactly 1 failure entry for each schema failure |
...
- [ ] Each schema-failure DLQ entry has `retry_count: 0` and one `retry_history` entry
